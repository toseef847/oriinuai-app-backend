...
async def ingest_book(book_id: str, file_bytes: bytes | None = None, use_day_chunking: bool = False) -> dict:
    ...
    This function now handles the initial storage upload to keep the API non-blocking.
    """
    try:
        ...
        if file_bytes:
            try:
                ...
                is_duplicate = "already exists" in error_str or "duplicate" in error_str
                is_lib_bug = "local variable 'response'" in error_str
                
                if is_duplicate or is_lib_bug:
        ...
        if use_day_chunking:
            ...
            if len(day_chunks) < 10:
                print(f"Warning: Only {len(day_chunks)} day chunks found. Falling back to word chunking.")
        ...
        for i in range(0, len(chunk_contents), batch_size):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(f"Embedded {min(i + batch_size, len(chunk_contents))}/{len(chunk_contents)} chunks...")
                    break
