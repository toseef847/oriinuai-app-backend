...
from app.schemas.chat import ChatRenameRequest, MessageEditRequest, ResponseRefineRequest
import json
...
async def rename_chat(
    ...
    client = await get_async_admin_client()
    res = await client.table("chat_sessions").update(
...
async def delete_chat(
    ...
    res = await client.table("chat_sessions").delete().eq(
        "id", str(session_id)
...
async def _stream_chat_response(
    ...
    history_result = await client.table("chat_messages").select(
        "role, content"
    ).eq("session_id", str(session_id)).order("created_at", desc=True).limit(10).execute()
    ...
    async def generate():
        ...
        async for token in llm.stream_response(system_prompt, actual_user_message, history):
        ...
        if is_refinement and existing_assistant_msg_id:
            ...
            await client.table("chat_messages").update({
                "content": complete,
                "model_used": plan["llm_tier"]
            }).eq("id", str(existing_assistant_msg_id)).execute()
        else:
            # Save new messages
            # For new sessions or normal chat, we save the user message if it's not already saved
            # (In /chat it's not saved yet, in /edit it's already updated)
            # Actually, to be safe and clean, let's handle saving outside or specifically here.
            # For /chat: need to save BOTH user and assistant
            # For /edit: user is already updated, need to save ONLY assistant
            
            # Check if user message already exists (for /edit it does)
            # This is a bit complex due to how we call this. 
            # Let's simplify: /chat and /edit will handle their own user message saving/updating.
            # This helper will ONLY save the assistant message.
        ...
        await client.table("chat_sessions").update(
            {"updated_at": "now()"}
        ).eq("id", str(session_id)).execute()
        
        # Track usage
        await client.rpc("increment_usage", {"p_user_id": user_id, "p_tokens": 0}).execute()
...
async def chat(
    ...
    if not session_id:
        ...
        session = await client.table("chat_sessions").insert(
            {"user_id": user_id, "title": title}
        ).execute()
        session_id = session.data[0]["id"]
    else:
        session_res = await client.table("chat_sessions").select("id").eq(
            "id", str(session_id)
...
async def edit_message(
    ...
    msg_res = await client.table("chat_messages").select(
        "session_id, role, created_at, chat_sessions!inner(user_id)"
    ).eq("id", str(message_id)).eq("role", "user").execute()
    ...
    await client.table("chat_messages").delete().eq(
        "session_id", session_id
    ).gt("created_at", created_at).execute()
...
async def refine_response(
    ...
    msgs_res = await client.table("chat_messages").select(
        "id, role, content"
    ).eq("session_id", str(session_id)).order("created_at", desc=True).limit(2).execute()

    if not msgs_res.data or len(msgs_res.data) < 2:
        raise HTTPException(status_code=400, detail="Cannot refine: insufficient history.")

    last_msg = msgs_res.data[0]
    prev_msg = msgs_res.data[1]

    if last_msg["role"] != "assistant" or prev_msg["role"] != "user":
        raise HTTPException(status_code=400, detail="Last message must be from assistant to refine.")
